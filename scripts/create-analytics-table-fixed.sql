...
-- RLS Policies - Allow users to view analytics for businesses they submitted
CREATE POLICY "Allow users to view their business analytics"
    ...
    USING (
        EXISTS (
            ...
            AND bl.submitted_by_user_id = auth.uid()
        )
        OR
        EXISTS (
...
-- This creates sample analytics for the last 30 days for existing businesses
INSERT INTO business_analytics (business_id, date, profile_views, website_clicks, phone_clicks, email_clicks, direction_clicks)
SELECT 
...
WHERE bl.status = 'approved' OR bl.status IS NULL
