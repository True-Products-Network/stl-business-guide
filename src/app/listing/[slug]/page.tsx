...
import { Star, MapPin, Phone, Globe, Mail, Crown, BadgeCheck } from 'lucide-react';
...
export default async function ListingPage({ params }: ListingPageProps) {
  ...
  return (
    <main className="min-h-screen bg-gray-50">
      ...
      {/* Plan Type Banner */}
      <div className={`${isVip ? 'bg-gradient-to-r from-[#ffc107] to-[#f68712]' : 'bg-gradient-to-r from-[#54afe6] to-[#bb7ce4]'} text-white py-3`}>
        <div className="max-w-7xl mx-auto px-4 text-center">
          <div className="flex items-center justify-center space-x-2">
            {isVip ? <Crown className="w-5 h-5" /> : <BadgeCheck className="w-5 h-5" />}
            <span className="font-bold">{isVip ? 'VIP' : 'Premium'} Business Listing</span>
          </div>
        </div>
      </div>
      ...
      <div className="relative h-64 md:h-80 bg-gradient-to-br from-[#371a5b] to-[#bb7ce4]">
        {business.logo_url ? (
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <span className="text-white/30 text-8xl font-bold">{business.business_name[0]}</span>
          </div>
        )}
        ...
        <div className="absolute bottom-0 left-0 right-0">
          <div className="max-w-7xl mx-auto px-4 py-6">
            <span className="text-white/80 text-sm font-medium mb-2 block">{mainCategory}</span>
            <h1 className="text-3xl md:text-5xl font-bold text-white mb-2">{business.business_name}</h1>
            
            {/* Google Rating */}
            <div className="flex items-center">
              <Star className="w-5 h-5 text-[#ffc107] fill-current" />
              <span className="text-white font-semibold ml-1">{business.google_rating || '4.5'}</span>
              <span className="text-white/70 ml-1">({business.google_reviews_count || '0'} reviews)</span>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {business.description_long && (
              <div className="bg-white rounded-xl shadow-md p-6">
                <h2 className="text-xl font-bold text-[#371a5b] mb-4">Details</h2>
                <p className="text-gray-600 leading-relaxed whitespace-pre-line">{business.description_long}</p>
              </div>
            )}
            ...
            {business.categories && business.categories.length > 0 && (
              <div className="bg-white rounded-xl shadow-md p-6">
                <h2 className="text-xl font-bold text-[#371a5b] mb-4">Categories</h2>
                <div className="flex flex-wrap gap-2">
                  {business.categories.map((cat: {name: string}, idx: number) => (
                    <span
                      key={idx}
            ...
            {/* Contact Form */}
            <div className="bg-white rounded-xl shadow-md p-6">
              <h2 className="text-xl font-bold text-[#371a5b] mb-4">Contact This Business</h2>
          ...
          <div className="space-y-6">
            {/* Contact Info Card */}
            <div className="bg-white rounded-xl shadow-md p-6">
              <h2 className="text-xl font-bold text-[#371a5b] mb-4">Contact Information</h2>
                ...
                {business.phone && (
                  <div className="flex items-center">
                    ...
                    <div>
                      ...
                      <a href={`tel:${business.phone}`} className="text-gray-800 font-medium hover:text-[#54afe6]">
                        {business.phone}
                      </a>
                    </div>
                  </div>
                ...
                {business.email && (
                  <div className="flex items-center">
                    ...
                    <div>
                      ...
                      <a href={`mailto:${business.email}`} className="text-gray-800 font-medium hover:text-[#54afe6] truncate block max-w-[200px]">
                        {business.email}
                      </a>
                    </div>
                  </div>
            ...
            {/* Quick Actions */}
            <div className="bg-white rounded-xl shadow-md p-6">
              <h2 className="text-xl font-bold text-[#371a5b] mb-4">Quick Actions</h2>
            ...
            {otherCategories.length > 0 && (
              <div className="bg-white rounded-xl shadow-md p-6">
                <h2 className="text-xl font-bold text-[#371a5b] mb-4">Also Listed In</h2>
                <div className="flex flex-wrap gap-2">
                  {otherCategories.map((cat: {name: string}, idx: number) => (
                    <span
                      key={idx}
            ...
            <div className="bg-gradient-to-r from-[#54afe6]/10 to-[#bb7ce4]/10 rounded-xl p-6 border border-[#54afe6]/20">
              <h3 className="font-bold text-[#371a5b] mb-2">Own this business?</h3>
